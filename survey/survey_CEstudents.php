<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<!-- saved from url=(0052)http://www.auburn.edu/includes/template_subpage.html -->
<HTML><!-- InstanceBegin template="/Templates/1column.dwt.php" codeOutsideHTMLIsLocked="false" --><HEAD>
<!-- InstanceBeginEditable name="doctitle" -->
<TITLE>Auburn University - Career Development Services</TITLE>
<!-- InstanceEndEditable -->
<META http-equiv=Content-Type content="text/html; charset=iso-8859-1">
<LINK 
media=screen href="../templates/ddcss_all.css" 
type=text/css rel=stylesheet>
<LINK media=print 
href="../templates/ddcss_print.css" type=text/css 
rel=stylesheet>
<LINK href="http://www.auburn.edu/includes/images/favicon.ico" 
rel="shortcut icon">
<META content="MSHTML 6.00.2900.2873" name=GENERATOR>
<script language="JavaScript" type="text/JavaScript">
<!--



function MM_preloadImages() { //v3.0
  var d=document; if(d.images){ if(!d.MM_p) d.MM_p=new Array();
    var i,j=d.MM_p.length,a=MM_preloadImages.arguments; for(i=0; i<a.length; i++)
    if (a[i].indexOf("#")!=0){ d.MM_p[j]=new Image; d.MM_p[j++].src=a[i];}}
}
//-->
</script>
<!-- InstanceBeginEditable name="head" --><!-- InstanceEndEditable --><!-- InstanceParam name="picture" type="boolean" value="true" --><!-- InstanceParam name="photo" type="boolean" value="true" --><!-- InstanceParam name="Photo" type="boolean" value="true" -->
</HEAD>
<BODY>
<TABLE cellSpacing=0 cellPadding=0 width=770 align=center border=0>
  <TBODY>
  <TR>
    <TD width=205 height=110 align=middle valign="top" bgColor=#03244d>
      <DIV>
        <div align="center"><A 
      style="BORDER-RIGHT: medium none; BORDER-TOP: medium none; BORDER-LEFT: medium none; BORDER-BOTTOM: medium none" 
      href="http://www.auburn.edu/"><IMG title="Auburn University" 
      style="MARGIN-TOP: 3px; MARGIN-BOTTOM: 5px" height=68 
      alt="Auburn University" 
      src="../templates/ddLogoEmblem2.gif" 
      width=113 border=0></A></div>
      </DIV><!--<div><img src="http://www.auburn.edu/includes/ddLogoEmblem.gif" alt="Auburn University" width="104" height="90" style="margin-top:7px;margin-bottom:7px;"></div> -->
      <DIV>        
        <div align="center"><IMG height=28 alt="Auburn University" 
      src="../templates/ddLogoAuburn.gif" 
      width=177></div>
      </DIV></TD>
    <TD width=565 vAlign=top 
    background="../images/banner.jpg" 
    bgColor=#052c5c>&nbsp; </TD>
  </TR>
  <TR>
    <TD vAlign=top width=205 bgColor=#03244d>
      <div align="right"><IMG height=2 
      src="../templates/ddLogoBar.gif" 
      width=190></div>
      <DIV style="MARGIN-TOP: 8px; MARGIN-BOTTOM: 8px" align=center><IMG 
      height=15 alt="Auburn University" 
      src="../templates/ddLogoUniversity.gif" 
      width=139></DIV></TD>
    <TD vAlign=top align=middle bgColor=#03244d>
      <DIV class=orangeDecorBar></DIV>
      <DIV id=topLinks align=center><A 
      href="http://www.auburn.edu/main/sitemap.php">Auburn A-Z Index</A> <IMG height=9 
      src="../templates/ddTopLinkDot.gif" 
      width=2> <A href="https://oitapps.auburn.edu/campusmap/">Campus Map </A> 
      <IMG height=9 
      src="../templates/ddTopLinkDot.gif" 
      width=2> <A 
      href="http://www.auburn.edu/main/auweb_campus_directory.html">People Finder </A> <IMG height=9 
      src="../templates/ddTopLinkDot.gif" 
      width=2> <A 
      href="http://www.auburn.edu/includes/template_subpage.html#">Search AU </A></DIV></TD></TR>
  <TR>
    <TD colSpan=2>
      <DIV class=orangeDecorBar 
  style="BACKGROUND-COLOR: #e1e1e1"></DIV></TD></TR></TBODY></TABLE>
<TABLE cellSpacing=0 cellPadding=0 width=770 align=center border=0>
  <TBODY>
  <TR>
    <TD id=contentArea style="PADDING-TOP: 6px" vAlign=top width=770 
    bgColor=#ffffff colSpan=2><!-- BEGIN CONTENT AREA -->
      <table width="750" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#f9903f">
        <tr>
          <td><div align="center"><a href="http://www.auburn.edu/career" class="cdsmenulinkstop">CDS HOME</a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/students" class="cdsmenulinkstop">STUDENTS</a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/employers" class="cdsmenulinkstop">EMPLOYERS</a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/alumni" class="cdsmenulinkstop">ALUMNI</a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/faculty" class="cdsmenulinkstop">FACULTY &amp; STAFF </a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/parents" class="cdsmenulinkstop">PARENTS</a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/career/aboutus" class="cdsmenulinkstop">ABOUT US</a> </div></td>
        </tr>
      </table>
      <br>
      <table width="750" border="0" cellspacing="0" cellpadding="10">
        <tr>
          <td valign="top"><!-- InstanceBeginEditable name="Body" -->
            <p><span class="cdsheads">Career Expo</span><br />
              The Career Expo is a significant  service provided by Career Development Services (CDS). In an effort to make  sure we are meeting the needs of our students, we would like you to answer a  few questions to let us know about your experience at the Career Expo. Please take  a few moments to fill out this questionnaire and feel free to add your  comments at the end.  We look forward to serving you in the future. Thank you.</p>
              <hr>
              <p><strong>All Information will be kept confidential and used only to improve services. </strong></p>
              <form id="form1" name="form1" method="post" action="actionCEstudents.php">
  <table width="672" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td width="672">&nbsp;</td>
    </tr>
    <tr>
      <td><p>1. Why did you visit the Career Expo? <strong>(Check all that apply)</strong>:</p>
        <table width="662" border="0">
                  <tr>
                    <td width="20"><input name="reason1" type="checkbox" id="reason1" value="1"></td>
                    <td width="176">Seeking a Job  </td>
                    <td width="20"><input name="reason2" type="checkbox" id="reason2" value="1"></td>
                    <td width="202">Seeking an Internship </td>
                    <td width="20"><input name="reason3" type="checkbox" id="reason3" value="1"></td>
                    <td width="198">Class Assignment </td>
                  </tr>
                  <tr>
                    <td><input name="reason4" type="checkbox" id="reason4" value="1"></td>
                    <td>Other</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td colspan="6">If you selected OTHER, please explain.<br>
                      <textarea name="other1" cols="75" id="other1"></textarea></td>
                    </tr>
                  <tr>
                    <td colspan="6"><hr></td>
                    </tr>
                  <tr>
                    <td colspan="6">2. How did you find out about The Career Expo? <strong>(Check all that apply)</strong> </td>
                    </tr>
                  <tr>
                    <td><input name="how1" type="checkbox" id="how1" value="1"></td>
                    <td>Class Assignment </td>
                    <td><input name="how2" type="checkbox" id="how2" value="1"></td>
                    <td>Student/Friend</td>
                    <td><input name="how3" type="checkbox" id="how3" value="1"></td>
                    <td><em>Plainsman</em> Ad </td>
                  </tr>
                  <tr>
                    <td><input name="how4" type="checkbox" id="how4" value="1"></td>
                    <td>Flyer</td>
                    <td><input name="how5" type="checkbox" id="how5" value="1"></td>
                    <td>Presentation</td>
                    <td><input name="how6" type="checkbox" id="how6" value="1"></td>
                    <td>Painted window or marquee </td>
                  </tr>
                  <tr>
                    <td><input name="how7" type="checkbox" id="how7" value="1"></td>
                    <td>Parents</td>
                    <td><input name="how8" type="checkbox" id="how8" value="1"></td>
                    <td>Faculty/Staff</td>
                    <td><label>
                      <input name="how9" type="checkbox" id="how9" value="1">
                    </label></td>
                    <td>Banner around campus </td>
                  </tr>
                  <tr>
                    <td><label>
                      <input name="how10" type="checkbox" id="how10" value="1">
                    </label></td>
                    <td>ThisWeek@AU email </td>
                    <td><label>
                      <input name="how11" type="checkbox" id="how11" value="1">
                    </label></td>
                    <td>Email from CDS </td>
                    <td><label>
                      <input name="how12" type="checkbox" id="how12" value="1">
                    </label></td>
                    <td>Email from AU entitiy </td>
                  </tr>
                  <tr>
                    <td><label>
                      <input name="how13" type="checkbox" id="how13" value="1">
                    </label></td>
                    <td>Other</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                  </tr>
                  <tr>
                    <td colspan="6">If you selected OTHER, please explain.<br />
                      <textarea name="other2" cols="75" id="other2"></textarea></td>
                    </tr>
                </table>
                <p></p>    </tr>
    <tr>
      <td>        3. I was very satisfied with the Career Expo. <br />
        <table width="633">
          <tr>
            <td width="20"><label>
              <input type="radio" name="three" value="1" />
            </label></td>
              <td width="136"> Strongly Agree </td>
              <td width="20"><label>
              <input type="radio" name="three" value="2" />
              </label></td>
              <td width="65"> Agree </td>
              <td width="20"><label>
              <input type="radio" name="three" value="3" />
              </label></td>
              <td width="96"> Disagree </td>
              <td width="21"><label>
              <input type="radio" name="three" value="4" />
              </label></td>
              <td width="219">Strongly Disagree </td>
          </tr>
        </table>      </td>
    </tr>
    <tr>
      <td>4. How many employers did you speak with at the event? <br />
        <table width="633">
          <tr>
            <td width="20"><label>
            <input type="radio" name="four" value="&lt;5" />
            </label></td>
            <td width="136">&lt;5 </td>
            <td width="20"><label>
              <input type="radio" name="four" value="5-10" />
            </label></td>
            <td width="65">5-10 </td>
            <td width="20"><label>
              <input type="radio" name="four" value="11-20" />
            </label></td>
            <td width="96"> 11-20 </td>
            <td width="21"><label>
              <input type="radio" name="four" value="21+" />
            </label></td>
            <td width="219"> 21+ </td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td><br>
        5. Did you obtain one of the following through this event? 
          <table width="400" border="0">
            <tr>
              <td width="36"><label>
                <input name="fivea" type="checkbox" id="fivea" value="full-time interview">
              </label></td>
              <td width="354">Interview for Full-time Job </td>
            </tr>
            <tr>
              <td><label>
                <input name="fiveb" type="checkbox" id="fiveb" value="internship interview">
              </label></td>
              <td>Interview for Internship </td>
            </tr>
            <tr>
              <td><label>
              <input name="fivec" type="checkbox" id="fivec" value="full-time job">
              </label></td>
              <td>Full-time Job </td>
            </tr>
            <tr>
              <td><label>
                <input name="fived" type="checkbox" id="fived" value="internship">
              </label></td>
              <td>Internship</td>
            </tr>
            <tr>
              <td><label>
                <input name="fivee" type="checkbox" id="fivee" value="waiting">
              </label></td>
              <td>Still waiting to hear </td>
            </tr>
            <tr>
              <td><label>
                <input name="fivef" type="checkbox" id="fivef" value="none">
              </label></td>
              <td>None of the above </td>
            </tr>
            <tr>
              <td><label>
                <input name="fiveg" type="checkbox" id="fiveg" value="other">
              </label></td>
              <td>Other </td>
            </tr>
          </table>
          <br>
          If you selected Other, please explain.<br>
          <textarea name="other3" cols="75" id="other3"></textarea> 
          <br />
        <table width="633">
          
        </table></td>
    </tr>
    <tr>
      <td><br>        
                6. If I need additional career assistance, I will seek further help from CDS.<br />
                <table width="633">
                  <tr>
                    <td width="20"><label>
                      <input type="radio" name="six" value="1" />
                    </label></td>
                    <td width="136">Strongly Agree </td>
                    <td width="20"><label>
                      <input type="radio" name="six" value="2" />
                    </label></td>
                    <td width="65">Agree </td>
                    <td width="20"><label>
                      <input type="radio" name="six" value="3" />
                    </label></td>
                    <td width="96">Disagree </td>
                    <td width="21"><label>
                      <input type="radio" name="six" value="4" />
                    </label></td>
                    <td width="219">Strongly Disagree </td>
                  </tr>
                </table></td>
    </tr>
    <tr>
      <td><br>
        7. I would recommend the Carer Expo to other students. <br />
        <table width="633">
          <tr>
            <td width="20"><label>
              <input type="radio" name="seven" value="1" />
            </label></td>
            <td width="136">Strongly Agree </td>
            <td width="20"><label>
              <input type="radio" name="seven" value="2" />
            </label></td>
            <td width="65">Agree </td>
            <td width="20"><label>
              <input type="radio" name="seven" value="3" />
            </label></td>
            <td width="96">Disagree </td>
            <td width="21"><label>
              <input type="radio" name="seven" value="4" />
            </label></td>
            <td width="219">Strongly Disagree </td>
          </tr>
        </table></td>
    </tr>
    <tr>
      <td><p><br>
        8. Please provide any additional comments/feedback as necessary.<br />
        <textarea name="comments" cols="75" id="comments"></textarea>
      </p>
        <p>Auburn email address: 
          <input name="email" type="text" id="email"><input name="date" type="hidden" value="<? echo $date=date("m/d/y   h:i A"); ?>">
</p></td>
    </tr>
    <tr>
      <td><input type="submit" name="Submit" value="Submit" /></td>
    </tr>
  </table>
</form><!-- InstanceEndEditable --></td>
          </tr>
      </table>
      <br>
      <table width="750" border="0" align="center" cellpadding="2" cellspacing="0" bgcolor="#f9903f">
        <tr>
          <td><div align="center"><a href="http://www.auburn.edu/academicsupport" class="cdsmenulinkstop">Academic Support </a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/fye" class="cdsmenulinkstop">Freshman Year Experience </a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu/scs" class="cdsmenulinkstop">Student Counseling </a></div></td>
          <td><div align="center"><a href="http://www.auburn.edu" class="cdsmenulinkstop">Auburn University </a></div></td>
        </tr>
      </table>
      <!-- END CONTENT AREA --></TD>
  </TR></TBODY></TABLE>
<TABLE cellSpacing=0 cellPadding=0 width=770 align=center border=0>
  <TBODY>
  <TR>
    <TD colSpan=2>
      <DIV class=orangeDecorBar></DIV></TD></TR>
  <TR>
    <TD id=footer align=middle colSpan=2><BR><BR>
    Career Development Services | A Division of Undergraduate Studies <br>    
    &nbsp;303 Mary Martin Hall | Auburn, Alabama 36849 &nbsp;| &nbsp;Phone: (334) 844-4744&nbsp;| 
      &nbsp;Email: cdsserv@auburn.edu
      <SCRIPT language=JavaScript type=text/javascript>
		cd<!-- 
		emailE='cdsserv@auburn.edu'
		emailE=('webmaster' + '@' + emailE)
		document.write('<A href="mailto:' + emailE + '">' + emailE + '</a>')
		//-->
		</SCRIPT>
       <BR>©
      <SCRIPT language=JavaScript type=text/javascript>
		<!--
		date = new Date();
		document.write(date.getFullYear());
		//-->
	  	</SCRIPT>
       <A href="http://www.auburn.edu/it_policies/copyright.html">Copyright 
Regulations</A></TD></TR></TBODY></TABLE></BODY><!-- InstanceEnd --></HTML>
